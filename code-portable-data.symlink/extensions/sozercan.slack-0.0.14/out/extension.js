...
class Slack {
    GetChannelList(callback, type, data) {
...
function reloadConfiguration() {
    ...
    if (NEW_TEAM_TOKEN) {
        disposables.push(slack = new Slack());
    }
    else {
...
function activate(context) {
    ...
    // snooze controls
    vscode.commands.registerCommand('slack.slackSetSnooze', () => slack.SetSnooze()), vscode.commands.registerCommand('slack.slackEndSnooze', () => slack.EndSnooze()));
    // reload configuration, when changed
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(() => reloadConfiguration()));
    reloadConfiguration();
}
